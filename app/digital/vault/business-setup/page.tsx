...
const setupSteps = [
  { id: 1, title: "Formalization & CIPC", desc: "Register entity via BizPortal and draft MOI." },
    ...
    <div className="min-h-screen bg-[#032213] text-[#F5D36B] font-sans p-8">
      ...
      <main className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-4">
          {setupSteps.map(step => (
            <div 
              ...
              className={`p-6 border rounded cursor-pointer transition ${completed.includes(step.id) ? 'bg-[#04381F] border-[#10B981]' : 'bg-[#021A0E] border-[#10B981]/20'}`}
            >
        ...
        <section className="bg-[#021A0E] p-6 border border-[#F5D36B]/20 rounded h-fit">
          ...
          <div className="w-full bg-[#032213] h-2 mb-4">
            <div className="bg-[#10B981] h-2" style={{ width: `${(completed.length / setupSteps.length) * 100}%` }}></div>
          </div>
          <p className="text-[10px] text-[#F5D36B]/60 font-mono">
            {completed.length} / {setupSteps.length} PROTOCOLS INITIALIZED
          </p>
        </section>
      </main>
    </div>

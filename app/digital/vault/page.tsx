...
export default function VaultMasterDashboard() {
  const vaultAssets = [
    ...
    },
    {
      ...
      tag: "COMPLIANCE // FOUNDATION",
      description: "Automated tracking for dividend purification, ensuring 100% compliant reinvestment and philanthropic flow.",
      status: "SYS_ONLINE",
      isLocked: false,
      price: "R 2,500",
    ...
    },
    {
      ...
      tag: "OPERATIONS // SA_ENTITIES",
      description: "Step-by-step diagnostic for CIPC incorporation, SARS fiscal setup, and Xero accounting automation.",
      status: "SYS_ONLINE",
      isLocked: false,
      price: "R 1,950",
      link: "/digital/vault/business-setup",
      checkoutLink: "#" 
    }
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {vaultAssets.map((asset) => (
            <div key={asset.id} className={`p-8 rounded-lg bg-[#021A0E] border ${asset.isLocked ? 'border-[#EF4444]/30' : 'border-[#10B981]/20'} hover:border-[#F5D36B]/40 transition duration-500 shadow-2xl flex flex-col justify-between relative overflow-hidden`}>
              {asset.isLocked && <div className="absolute -right-8 -bottom-8 opacity-5 text-9xl">🔒</div>}
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-4">
                  <span className="px-2 py-0.5 text-[10px] font-mono tracking-wider bg-[#032213] text-[#10B981] border border-[#10B981]/30 rounded">{asset.tag}</span>
                  <span className={`flex items-center gap-1.5 text-[10px] font-mono ${asset.isLocked ? 'text-[#EF4444]' : 'text-[#10B981]'}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${asset.isLocked ? 'bg-[#EF4444]' : 'bg-[#10B981]'}`}></span>{asset.status}
                  </span>
                </div>
                <h3 className="text-2xl font-serif tracking-wide text-[#FFFDF0] mb-3">{asset.title}</h3>
                <p className="text-sm text-[#F5D36B]/70 font-light leading-relaxed mb-8">{asset.description}</p>
              </div>
              <div className="pt-6 border-t border-[#10B981]/10 relative z-10">
                {asset.isLocked ? (
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <a href={asset.checkoutLink} className="flex-1 text-center px-4 py-3 bg-[#F5D36B] text-[#032213] text-xs uppercase tracking-widest font-mono font-bold hover:bg-[#FFFDF0] transition shadow-[0_0_15px_rgba(245,211,107,0.15)]">Unlock Engine</a>
                  </div>
                ) : (
                  <a href={asset.link} className="block w-full text-center px-4 py-3 bg-[#10B981]/10 border border-[#10B981]/30 text-[#10B981] text-xs uppercase tracking-widest font-mono font-bold hover:bg-[#10B981] hover:text-[#032213] transition">Initialize Engine</a>
                )}
              </div>
            </div>
          ))}
